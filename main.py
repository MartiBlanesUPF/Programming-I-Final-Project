...
def test_data_loading_and_cleaning():
    ...
    required_columns = [
        "Id", "SalePrice", "LotArea", "YearBuilt", "BedroomAbvGr"
    ]
    assert loader.validate_columns(
        required_columns), "Required columns missing from dataset"

    # Load and test data format
    data = loader.load_data_from_csv()
    assert isinstance(data, list), "Data should be returned as a list"
    assert all(isinstance(row, dict)
               for row in data), "Each row should be a dictionary"
    ...
    assert all((key.islower() and "_") or key.islower() in key for key in
               cleaned_data[0].keys()), "Column names should be in snake_case"
...
def test_descriptive_statistics(cleaned_data: List[Dict[str, Any]]):
    ...
    assert isinstance(none_ratios,
                      dict), "None ratios should be returned as dictionary"
    assert set(none_ratios.keys()) == set(none_ratios_numpy.keys(
    )), "Both implementations should handle same columns"

    # Test numeric calculations
    numeric_columns = ["sale_price",
                       "lot_area"]  # Assuming these are the cleaned names
    ...
    for col in numeric_columns:
        assert abs(averages[col] - averages_numpy[col]
                   ) < 1e-6, f"Average calculations differ for {col}"
        assert abs(
            medians[col] -
            medians_numpy[col]) < 1e-6, f"Median calculations differ for {col}"
        assert abs(percentiles[col] - percentiles_numpy[col]
                   ) < 1e-6, f"Percentile calculations differ for {col}"

    # Test type and mode
    type_modes = descriptor.type_and_mode()
    type_modes_numpy = descriptor_numpy.type_and_mode()
    assert set(type_modes.keys()) == set(type_modes_numpy.keys(
    )), "Both implementations should handle same columns"
...
def test_house_functionality():
    ...
    assert isinstance(price_per_sqft,
                      float), "Price per square foot should be float"
    assert price_per_sqft == 100.0, "Incorrect price per square foot calculation"

    # Test new construction logic
    assert house.is_new_construction(
        2024) is False, "House should not be considered new construction"
    assert house.is_new_construction(
        2012) is True, "House should be considered new construction"
    ...
    house.available = True #so that test_consumer_functionality can find a house (it was living this house as 'not available' and test_consumer_functionality would fail if it didn't find a house)
...
def test_market_functionality(house: House):
    ...
    matching_houses = market.get_houses_that_meet_requirements(
        max_price=250000, segment=Segment.AVERAGE, annual_income=50000)
    assert isinstance(matching_houses,
                      list), "Should return list of matching houses"
...
def test_consumer_functionality(market: HousingMarket):
    ...
    assert consumer.house is not None or market.get_houses_that_meet_requirements(
        ...
        annual_income=consumer.annual_income
...
def test_simulation(cleaned_data: List[Dict[str, Any]]):
    ...
    simulation = Simulation(housing_market_data=cleaned_data,
                            ...
                            annual_income=AnnualIncomeStatistics(
                                ...
                                maximum=150000.0),
                            children_range=ChildrenRange(minimum=0, maximum=5),
    ...
    assert hasattr(simulation,
                   'housing_market'), "Housing market should be created"

    # Test consumer creation
    simulation.create_consumers()
    assert hasattr(simulation, 'consumers'), "Consumers should be created"
    assert len(simulation.consumers
               ) == 100, "Should create specified number of consumers"

    # Test savings computation
    simulation.compute_consumers_savings()
    assert all(
        c.savings > 0
        for c in simulation.consumers), "All consumers should have savings"
...
def test_house_price_predictor():
    ...
    try:
        ...
        print(f"Data cleaning failed: {traceback.format_exc()}")
        return
    ...
    try:
        ...
        print(f"Feature preparation failed: {traceback.format_exc()}")
        return
...
def main():
    ...
    try:
        ...
        print("Starting house price predictor test...")
